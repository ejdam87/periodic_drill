...
def load_table(path: str) -> tuple[list[tuple[str, str, int, int, int]], dict[str, str]]:
    ...
    with open(path, encoding="utf8") as f:
        ...
        for elem in table["order"]:
            ...
            elconf[table[elem]["symbol"]] = table[elem]["electron_configuration"]
...
class MainWindow(tk.Tk):
    ...
    def elconf_handle(self, symbol: str) -> None:
        elconf = ElconfWindow(self)
        elconf.configure(bg=BG_COLOR)
        elconf.configure_quiz(symbol, self.elconf[symbol])
    ...
    def fill(self, x: int, y: int) -> None:
        ...
        if self.active_element.get() == symbol:
            ...
            ## Elconf mode is active ?
            if self.elconf_active.get() == 1:
